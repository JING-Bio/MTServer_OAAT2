...
olink_long2wide <- function(raw_npx_data_path = NULL,
  ...
  if (is.null(target_npx_data) && is.null(raw_npx_data_path)  && !is.null(npx_data_path)) { # 需要处理的是成熟的NPX数据，NPX长数据Long Data
    ...
    ### data.frame subset
    npx_data_new <- npx_data[,c(firstrow, firstcol, cellvalue, "Panel")]
    ### head df
    ...
    # target_npx_data
    ### data.frame subset
    npx_data_new <- target_npx_data[,c(firstrow, firstcol, cellvalue, "Panel")]
    ### head df
    # print(head(npx_data_new))
    ### filtered samples
    filtered_samples_list <- unique(npx_data_new[[firstrow]][grepl(filtered_samples, npx_data_new[[firstrow]], ignore.case = F, perl = F, fixed = F, useBytes = F)])
  ...
  # SampleID           UniProt               NPX               Panel
  # Length:129448      Length:129448      Min.   :-15.06320   Length:129448
  # Class :character   Class :character   1st Qu.: -0.43930   Class :character
  # Mode  :character   Mode  :character   Median :  0.06900   Mode  :character
  # Mean   :  0.09864
  # 3rd Qu.:  0.60870
  # Max.   : 10.00050
  ### --------------------------------------------------------------------------
  ##############################################################################
  for (pl in unique(npx_clean_longdata$Panel)) { # 遍历所有Panel，保存Panel数据
    npx_clean_longdata_panel <- npx_clean_longdata[which(npx_clean_longdata$Panel == pl),]
    #
    samplelist <- unique(npx_clean_longdata_panel$SampleID)
    # print(paste0("Olink-NPX数据样本个数为：", length(samplelist)))

    proteinlist <- unique(npx_clean_longdata_panel$UniProt)
    # print(paste0("Olink-NPX数据蛋白个数为：", length(proteinlist)))

    firstcol_list <- samplelist

    data_part_df <- npx_clean_longdata_panel[which(npx_clean_longdata_panel[[firstrow]] == npx_clean_longdata_panel[[firstrow]][1]),]
    colnames(data_part_df)[3] <- npx_clean_longdata_panel[[firstrow]][1]
    ...
    for (fc in firstcol_list) {
      data_part <- npx_clean_longdata_panel[which(npx_clean_longdata_panel[[firstrow]] == fc),]
    ...
    if (!is.null(npx_data_path) && is.null(raw_npx_data_path) && is.null(target_npx_data)) { # npx_data_path不为空，即读取的是NPX的长数据
      ...
      rio::export(df_new, file = paste0(output_file_path,"/", output_wide_npx_file, "_", pl, "_Panel_WideData_", npx_wide_nrow, "x", npx_wide_ncol, ".xlsx"), overwrite = T)
      ...
      npx_wide_nrow <- NROW(df_new)
      npx_wide_ncol <- NCOL(df_new)
      rio::export(df_new, file = paste0(output_file_path,"/Olink_NPX_", pl, "_Panel_WideData_", npx_wide_nrow, "x", npx_wide_ncol,".xlsx"), overwrite = T)
    } else {
      break
    }
    #
  ...
  if (!is.null(npx_data_path) && is.null(raw_npx_data_path) && is.null(target_npx_data)) { # npx_data_path不为空，即读取的是NPX的长数据
    ...
    npx_wide_nrow <- NROW(df_new)
    npx_wide_ncol <- NCOL(df_new)
    rio::export(df_new, file = paste0(output_file_path,"/", output_wide_npx_file, "_ALL_Panel_NPX_WideData_", npx_wide_nrow, "x", npx_wide_ncol, ".xlsx"), overwrite = T)
  } else if (is.null(npx_data_path) && !is.null(raw_npx_data_path) && is.null(target_npx_data)) { # raw_npx_data_path不为空，即读取的是NPX的raw数据（非长数据或宽数据）
    output_wide_npx_file <- strsplit(basename(raw_npx_data_path), "[.]")[[1]][1]
    #### 获取NPX宽数据的行数（蛋白、基因）和列数（样本）
    npx_wide_nrow <- NROW(df_new)
    ...
    npx_wide_nrow <- NROW(df_new)
    npx_wide_ncol <- NCOL(df_new)
    rio::export(df_new, file = paste0(output_file_path,"/Olink_ALL_Panel_NPX_WideData_", npx_wide_nrow, "x", npx_wide_ncol,".xlsx"), overwrite = T)
  } else {
    break
  }
  #
...
# olink_long2wide(raw_npx_data_path = "/Users/mingma_10000455/P/OLINK项目/医创云康/医创云康88血浆_Raw_NPX_Data.xlsx",
#                 npx_data_path = NULL,
#                 target_npx_data = NULL,
#                 firstcol = "UniProt",
#                 firstrow = "SampleID",
#                 cellvalue = "NPX",
#                 filtered_samples = "Sample Control", # SC/Sample Control
